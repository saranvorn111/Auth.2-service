...
import com.developerscambodia.oauth2server.security.CustomUserDetails;
...
public class CustomPasswordAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        log.info("KANGCHI : {}", authentication.getName());
        ...
        try {
            ...
            log.error("KANGCHI : {}", e.getMessage());
            throw new OAuth2AuthenticationException(OAuth2ErrorCodes.ACCESS_DENIED);
        }

        log.info("KANGCHI - PASSWD : {}", customUserDetails.getPassword());
        log.info("KANGCHI - USER : {}", customUserDetails.getUsername());
        log.info("KANGCHI - PASSWD : {}", password);
        log.info("KANGCHI - USER : {}", username);
        log.info("KANGCHI - USER : {}", passwordEncoder.matches(password, customUserDetails.getPassword()));
        if (!passwordEncoder.matches(password, customUserDetails.getPassword()) || !customUserDetails.getUsername().equals(username)) {
            throw new OAuth2AuthenticationException(OAuth2ErrorCodes.ACCESS_DENIED);
        }

        authorizedScopes = customUserDetails.getAuthorities().stream()
        ...
        CustomUserPassword customPasswordUser = new CustomUserPassword(username, customUserDetails.getAuthorities());
